export async function carregarTextura(gl, caminho) {
  ...
  const imagem = await new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      reject(new Error(`Erro ao carregar a textura: ${caminho}`));
    };
...
// Carrega várias texturas em paralelo
export async function carregarTexturas(gl, caminhos) {
  const nomes = Object.keys(caminhos);
  const promessas = [];
  for (const nome of nomes) {
    promessas.push(carregarTextura(gl, caminhos[nome]));
  }

  const carregadas = await Promise.all(promessas);
  const texturas = {};
  for (let i = 0; i < nomes.length; i++) {
    texturas[nomes[i]] = carregadas[i];
  }
  return texturas;
}